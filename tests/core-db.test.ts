...
describe('CoreDB', () => {
  ...
  describe('Schema Management', () => {
    ...
    it('should create a new table', async () => {
      await expect(db.schemaCreateOrUpdate(userTableDef)).resolves.not.toThrow();
    });
    ...
    it('should update existing table with new fields', async () => {
      ...
      await expect(db.schemaCreateOrUpdate(updatedTableDef)).resolves.not.toThrow();
    });
    ...
    it('should rename a field', async () => {
      await db.schemaCreateOrUpdate(userTableDef);
      await expect(db.schemaRenameField('users', 'email', 'emailAddress')).resolves.not.toThrow();
    });
    ...
    it('should throw error for unsupported field type', async () => {
      const invalidTableDef: TableDefinition = {
        ...
        implementation: 'Static',
        fields: [
          { name: 'field', type: 'InvalidType' as any }
        ]
      };
  ...
  describe('Query Builder', () => {
    ...
    beforeEach(async () => {
      await db.schemaCreateOrUpdate(userTableDef);
      await db.insert('users', { name: 'John Doe', email: 'john@example.com', age: 30 });
      await db.insert('users', { name: 'Jane Doe', email: 'jane@example.com', age: 25 });
      await db.insert('users', { name: 'Bob Smith', email: 'bob@example.com', age: 35 });
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle invalid field values', async () => {
      ...
      await expect(db.insert('users', { age: 'not a number' })).rejects.toThrow();
    });
